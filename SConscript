...
env.Append(CPPPATH=env['build_dir'] + '/include/Compass')

#
# Need to include the Compass library for binaries.
#

libs = ['Compass'] + env.get('LIBS', [])
...
for d in dirs:
    ...
    src_dir = d + '/src'
    inc_dir = 'include/Compass/' + d
    tst_dir = d + '/tests__'
    tst_exp_dir = tst_dir + '/expected'
    tst_input_dir = tst_dir + '/input'
    tst_install_dir = 'tests/Compass/' + d
...
# Need to install headers from libCompass.
env.Install('include/Compass', Glob('aspect/src/*.h'))
...
for d in dirs:
    ...
    name = 'Compass_' + d.split('/')[-1] + 'module'
    mod_name = env['ESCAPE']('"' + ''.join(d.split('/')) + '"')
    cpp_defs = [('CURR_MODULE_NAME', mod_name)] + env.get('CPPDEFINES', [])

    env.Install('include/Compass/' + d.split('/')[-1], Glob(d + '/*.h'))
...
if env['shared_libs']:
    env.SharedLibrary('lib/Compass', objs)
...
if env['static_libs']:
    ...
    # Static library.
    l = env.StaticLibrary(env['build_dir'] + '/lib/Compass', objs + pl_objs + reg_obj)
...
env.PCUTest('tests/testCompass', 
            suites,
            PCU_LIBHEADERS="#include <Compass/Compass.h>\n",
            PCU_SETUP="coInit(&argc, &argv,coStaticAdvice);\n",
            PCU_TEARDOWN="coFinalise();",
            LIBS=libs, 
            PCU_EXP=tst_exp, 
            PCU_INPUT=tst_input, 
            PROJECT="Compass" )
